...
class BasketBallChat implements IGamePlayModel {
	...
	public addNetPlayer=(myInstance, rtcEvent?) => {
		...
		if(addToScene) {
			console.info("rtcEvent.connectionId]>> >>>>>>>>>>>>>>>>", rtcEvent.connectionId);
			netPlayer.id=rtcEvent.connectionId;
			...
			console.info("Net Player body created.", myInstance.netBodies["netObject_"+rtcEvent.connectionId]);
